...
| [2026-09-10](./2026-09-10-the-flat-allow-list-and-the-40-px-default.md) | Issues #10 and #11 cleared, and **the pull request opened at last** — [#12](https://github.com/sanama-stack/reception-booking-system/pull/12), fifteen commits, closing all six issues. Built no screen. Both issues were `ready-for-human`, so both were decided before being written: 44 px becomes the shared default rather than an override four files remember, and the public allow-list holds paths qualified by the response that produced them rather than a flat set of key names. The second was verified against its counterfactual — the old test passes green with a planted leak the new one names. 798 tests, unchanged, because this strengthened a test rather than adding one. |
